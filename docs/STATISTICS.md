...
| Day by day | "Is practice actually happening — and what did he work on each day?" | `daily` — one row PER DAY, never merged (words, accuracy, stars). Each row opens (clickable fold, like Recent sessions) to `words`: the words practiced that day, `{w, tries, wrong, group?, heart?}`, summed from the per-word `days` tallies (unaided), misses first. ✓ = never missed that day, ✗ = missed at least once. |
